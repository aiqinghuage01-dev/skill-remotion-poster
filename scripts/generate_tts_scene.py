#!/usr/bin/env python3
"""
Generate MiniMax cloned-voice TTS for scene-based scripts.
Adapts scene-script.json format to work with the TTS pipeline.

Usage:
  python3 scripts/generate_tts_scene.py src/data/<name>-scene-script.json public/audio/
"""
...
def main() -> int:
    if len(sys.argv) < 3:
        print("Usage: python3 scripts/generate_tts_scene.py <scene-script.json> <audio_dir>", file=sys.stderr)
        return 2
    ...
    # Support both "scenes" (new format) and "slides" (old format)
    items = script.get("scenes", script.get("slides", []))
    ...
    for i, item in enumerate(items):
        ...
        fallback_duration = float(item.get("duration", 2) or 2)
        ...
        print(f"Scene {i} [{item.get('type', '?')}]: 生成 MiniMax TTS「{narration[:40]}...」")
